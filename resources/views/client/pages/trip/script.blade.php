...
{{-- <script src="{{ asset('admin/assets/libs/list.paginations.js/list.paginations.min.js') }}"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/paginationsjs/2.1.5/paginations.min.js"></script> --}}
...
{{-- <script type="text/javascript">
    ...
    const itemsPage = 5; // Số mục hiển thị trên mỗi trang
    let currentPages = 1;
    let totalpages = 0;
    let startIndexs = 0;
        ...
        debounceTimer = setTimeout(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $searchResults.empty();
                    printedTripNames = [];
                    const endIndex = startIndexs + itemsPage;
                    const filteredData = data.slice(startIndexs, endIndex);
            ...
            const calculatetotalPages = (totalItems) => {
                totalPages = Math.ceil(totalItems / itemsPage);
            };
            ...
            const renderPaginations = () => {
                ...
                for (let i = 1; i <= totalPages; i++) {
                    paginationsHtml += `
                <li class="page-item ${i === currentPages ? 'active' : ''}">
                    <a class="page-link" href="#" onclick="changePage(${i})">${i}</a>
                </li>
            `;
                }

                paginations.innerHTML = paginationsHtml;

            };

            const changePage = (page) => {
                currentPages = page;
                startIndexs = (currentPages - 1) * itemsPage;
                getData();
            };
        }, 300);
