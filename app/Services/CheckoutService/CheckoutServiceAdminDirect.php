...
class CheckoutServiceAdminDirect
{
    ...
    public function __construct()
    {
        ...
        $this->vnp_ReturnUrl = route('status_checkout_seat-admin');
    }
    ...
    public function momoPayment($request)
    {
        ...
        $redirectUrl = route('status_checkout_seat-admin');
        $ipnUrl = route('status_checkout_seat-admin');
        $extraData = "";
    ...
    public function directPayment($request)
    {
        ...
        // Config data for my bill
        if ($request->money_return) {
            $totalMoneyDirect = ($request->money_turn + $request->money_return);
        } else {
            $totalMoneyDirect = $request->money_turn;
        }

        $direct_seatsReturn = null;
        if ($request->seats_return) {
            $direct_seatsReturn = implode(',', $request->seats_return);
        }
        $direct_seatsTurn = implode(',', $request->seats_turn);

        $direct_tripReturn = null;
        if ($request->trip_return) {
            $direct_tripReturn = $request->trip_return;
        }
        $direct_tripTurn = $request->trip_turn;

        $direct_startTurn_0 = $request->place_start_turn;
        $direct_endTurn_0 = $request->place_end_turn;
        $direct_startTurn_1 = null;
        $direct_endTurn_1 = null;
        if ($request->trip_return && $request->trip_turn) {
            $direct_startTurn_0 = $request->place_start_turn_0;
            $direct_endTurn_0 = $request->place_end_turn_0;
            $direct_startTurn_1 = $request->place_start_turn_1;
            $direct_endTurn_1 = $request->place_end_turn_1;
        }
        Cache::put('my_bill_cache' . $orderId, [
            'clientLogin' => $request->client_login,
            'email' => $request->phone,
            ...
            'moneyTurn' => $request->money_turn,
            'moneyReturn' => $request->money_return,
            'seatsTurn' => $direct_seatsTurn,
            'seatsReturn' => $direct_seatsReturn,
            'tripTurn' => $direct_tripTurn,
            'tripReturn' => $direct_tripReturn,
            'startTurn_0' => $direct_startTurn_0,
        ...
        // Config data for my bill
        // Information checkout
        return "http://127.0.0.1:8000/manage/order-ticket/status-checkout?directCode=0&totalMoneyDirect=" . $totalMoneyDirect . "&orderId=" . $orderId;
    }
}
